    ...
    local function CreateRow(data, isWoW)
        ...
        if isWoW then
            ...
            if data.faction == "Horde" then
                facIcon = "|TInterface\\Common\\icon-Horde:16:16:0:0|t"
            elseif data.faction == "Alliance" then
                facIcon = "|TInterface\\Common\\icon-Alliance:16:16:0:0|t"
            end
...
function Bar:GetSafeConfig(name)
    if not self.db.profile.brokers[name] then 
        self.db.profile.brokers[name] = { bar = "MainBar", align = "CENTER", order = 10, showIcon = true, showText = true, showLabel = false, showCoords = false } 
    end
...
function Bar:CreateWidget(name, obj)
    ...
    LDB.RegisterCallback(self, "LibDataBroker_AttributeChanged_"..name, function() Bar:UpdateBarLayout("MainBar") end)
end
...
function Bar:MoveBroker(name, direction)
    local config = self:GetSafeConfig(name)
    if not config or config.bar == "None" then return end
    local list = {}
    ...
    if target >= 1 and target <= #list then 
        local tName = list[target].name
        self.db.profile.brokers[name].order, self.db.profile.brokers[tName].order = self.db.profile.brokers[tName].order, self.db.profile.brokers[name].order
